	...
	mejs.MediaElementPlayer.prototype = {
		...
		showControls: function(doAnimation) {
			...
			if (doAnimation) {
				...
				t.container.parent().find('.mejs-control')
					.css('visibility','visible')
		...
		hideControls: function(doAnimation) {
			...
			if (doAnimation) {
				...
				// any additional controls people might add and want to hide
				t.container.parent().find('.mejs-control').stop(true, true).fadeOut(200, function() {
					$(this)
						.css('visibility','hidden')
						.css('display','block');
				});
		...
		meReady: function(media, domNode) {
			...
			if (!(mf.isAndroid && t.options.AndroidUseNativeControls) && !(mf.isiPad && t.options.iPadUseNativeControls) && !(mf.isiPhone && t.options.iPhoneUseNativeControls)) {
				...
				if (!$(media).is(':visible')) {
					var mediaVisible = setInterval(function() {
						if ($(media).is(':visible')) {
							t.setPlayerSize(t.width, t.height);
							t.setControlsSize();
							clearInterval(mediaVisible);
						}
					}, 1)
				}
				...
				t.globalBind('resize', function() {
					...
					if ( !(t.isFullScreen || (mejs.MediaFeatures.hasTrueNativeFullScreen && document.webkitIsFullScreen)) && $(media).is(':visible')) {
						t.setPlayerSize(t.width, t.height);
					}
		...
		setPoster: function(url) {
			...
			posterDiv.css({'background-image' : 'url(' + url + ')', 'background-size' : 'cover'});
		},
		...
		buildkeyboard: function(player, controls, layers, media) {
				...
				t.globalBind('keydown', function(event) {
					//overplayer.hasFocus = $(event.target).closest('.mejs-container').length !== 0;
